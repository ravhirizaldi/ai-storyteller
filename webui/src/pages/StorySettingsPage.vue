<template>
  <div class="flex-1 overflow-y-auto">
    <!-- Header -->
    <div
      class="sticky top-0 bg-parchment-50/95 backdrop-blur-sm border-b border-ink-100 px-8 py-4 z-10"
    >
    ...
    <div v-else class="max-w-2xl mx-auto px-8 py-6 space-y-6">
      ...
      <div class="card p-6 space-y-4">
        <h2 class="font-semibold text-ink-800 text-sm uppercase tracking-wider">
          Informasi Dasar
        </h2>
      ...
      <div class="card p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            >
              ...
              <option value="progress_story">
                Progress Story (Lebih Cepat)
              </option>
        ...
        <div class="space-y-2 pt-2 border-t border-ink-100">
          ...
          >
            <input
              type="checkbox"
              v-model="form.scene_lock"
              class="rounded border-ink-300 text-sage-600 focus:ring-sage-500"
            />
          ...
          >
            <input
              type="checkbox"
              v-model="form.allow_location_change"
              class="rounded border-ink-300 text-sage-600 focus:ring-sage-500"
            />
          ...
          >
            <input
              type="checkbox"
              v-model="form.allow_major_plot_progress"
              class="rounded border-ink-300 text-sage-600 focus:ring-sage-500"
            />
      ...
      <div class="card p-6 border-red-200 space-y-6">
        <div>
          <h2
            class="font-semibold text-red-700 text-sm uppercase tracking-wider mb-3"
          >
            Reset Progress Cerita
          </h2>
          <p class="text-xs text-ink-500 mb-4">
            Ini akan menghapus seluruh riwayat pesan & ingatan, mengosongkan
            kondisi adegan, dan mengembalikan System Prompt + pengaturan
            generasi (mode, temperature, scene lock, dll) ke default terbaru.
            Judul, Story Bible, dan Karakter TETAP dipertahankan (current state
            karakter di-reset).
          </p>
          <button
            class="btn-secondary btn-sm text-red-600 border-red-200 hover:bg-red-50"
            @click="confirmReset"
            id="btn-reset-story"
          >
        ...
        <div class="pt-6 border-t border-red-100">
          ...
          <p class="text-xs text-ink-500 mb-4">
            Menghapus cerita akan menghapus semua pesan, karakter, memori, dan
            data terkait secara permanen. Tindakan ini tidak bisa dibatalkan.
          </p>
          <button
...
async function confirmReset() {
  if (
    !confirm(
      `Reset progress cerita "${currentStory.value?.title}"? Pesan dan memori akan hilang, tapi pengaturan dan karakter tetap dipertahankan.`,
    )
  )
    return;
  try {
...
async function confirmDelete() {
  if (
    !confirm(
      `Hapus cerita "${currentStory.value?.title}"? Tindakan ini tidak bisa dibatalkan.`,
    )
  )
    return;
  try {
