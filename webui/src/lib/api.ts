...
export const messagesApi = {
  list: async (storyId: string, cursor?: string) => {
    const url = new URL(
      `${BASE_URL}/api/stories/${storyId}/messages`,
  ...
  deleteLastExchange: (storyId: string) =>
...
export const charactersApi = {
  ...
  create: (
    storyId: string,
    data: Omit<Character, "id" | "story_id" | "created_at" | "updated_at">,
  ) =>
    request<Character>(`/api/stories/${storyId}/characters`, {
      method: "POST",
      body: JSON.stringify(data),
...
export const memoriesApi = {
  ...
  create: (
    storyId: string,
    data: { type: string; content: string; importance?: number },
  ) =>
    request<StoryMemory>(`/api/stories/${storyId}/memories`, {
      method: "POST",
      body: JSON.stringify(data),
...
export const plotThreadsApi = {
  list: (storyId: string) =>
    request<PlotThread[]>(`/api/stories/${storyId}/plot-threads`),
  create: (
    storyId: string,
  ...
  update: (id: string, data: Partial<PlotThread>) =>
    request<PlotThread>(`/api/plot-threads/${id}`, {
      method: "PATCH",
      body: JSON.stringify(data),
    }),
  delete: (id: string) =>
    request<void>(`/api/plot-threads/${id}`, { method: "DELETE" }),
};
